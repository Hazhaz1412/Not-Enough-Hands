...
## Proves the baked clips actually move Jeff's visible skeleton, which is the
## only thing that matters here: jeff_the_killer.glb ships no animations, so the
## library is retargeted from other rigs and a bone-name or track-path slip would
## leave AnimationPlayer happily "playing" a clip that drives nothing.
##
##   godot --headless --script tests/jeff_animation_smoke.gd

const VISUAL := "res://ghosts/jeff_ghost_visual.tscn"
...
func _run() -> void:
	var visual: JeffGhostVisual = (load(VISUAL) as PackedScene).instantiate()
	root.add_child(visual)

	var player := visual.model.find_child("AnimationPlayer", true, false) as AnimationPlayer
	if not player:
		return _fail("JeffGhostVisual built no AnimationPlayer")
	var skeleton := visual.model.find_child("Skeleton3D", true, false) as Skeleton3D
	if not skeleton:
		return _fail("no Skeleton3D under the Jeff model")
	...
	for clip_name in EXPECTED:
		...
		for i in anim.get_track_count():
			...
			if skeleton.find_bone(bone) == -1:
				return _fail("'%s' drives bone '%s', which the Jeff skeleton does not have"
					% [clip_name, bone])
	...
	print("PASS jeff_animation_smoke")
	quit()
